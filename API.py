...
def __build_query__(location, term, offset=False):  # Builds queries to fetch restaurant data
    if not offset:
        query = '{search (location: "' + location + '", term: "' + term + '", sort_by: "rating", limit: 30) {'
    else:
        query = '{search (location: "' + location + '", term: "' + term + '", offset:' + str(
            offset) + ', sort_by: "rating" open_now: true, limit: 30) {'

    query += ' total business{id name rating price}}}'
...
def __select_tasty_spots__(data, collection=[]):
    for restaurant in data['business']:
        ...
        if restaurant['rating'] >= 4.5:
            collection.append([restaurant['id'], restaurant['name'], restaurant['rating'], restaurant['price']])
...
class API:
    def __init__(self):  # Initialize object by connecting to API
        apiKey = "YELP GRAPHQL TOKEN GOES HERE"
    ...
    def getRestaurants(self, location, term):
        query = __build_query__(location, term)
        ...
        while len(restaurants) < 7 and (offset + 30) < total:
            offset += 30
            try:
                query = __build_query__(location, term, offset)
